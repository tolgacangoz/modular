...
class FlowMatchEulerDiscreteScheduler:
    ...
    def index_for_timestep(
        self, timestep: Tensor | float, schedule_timesteps: Tensor | None = None
    ) -> int:
        # Get timestep value as Python float
        if isinstance(timestep, Tensor):
            timestep_val = float(timestep.to(CPU()))
        else:
            timestep_val = float(timestep)

        # Use pre-computed Python list for sync-free lookup
        timesteps_list = self.timesteps_list

        # Find matches within tolerance
        matches = [
            i
            for i, t in enumerate(timesteps_list)
            if abs(t - timestep_val) < 1e-5
        ]

        if len(matches) == 0:
            # Fallback to argmin
            diffs = [abs(t - timestep_val) for t in timesteps_list]
            return diffs.index(min(diffs))

        # We want the second match (pos=1) if multiple matches, else first (pos=0)
    ...
    def step(
        ...
        if per_token_timesteps is not None:
            ...
            sigma = self.sigmas_list[sigma_idx]
            sigma_next = self.sigmas_list[sigma_idx + 1]
