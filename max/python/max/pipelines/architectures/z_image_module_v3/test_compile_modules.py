...
def test_attention():
    ...
    hidden_states_type = TensorType(
        DType.bfloat16, shape=(B, S, C), device=device_ref
    )
    freqs_cis_type = TensorType(
        DType.float32, shape=(B, S, head_dim // 2, 2), device=device_ref
    )
...
def test_transformer_block():
    ...
    adaln_type = TensorType(
        DType.bfloat16, shape=(B, adaln_dim), device=device_ref
    )
...
def main():
    ...
    for name, result in results.items():
        status = (
            "PASS"
            if result == True
            else ("HANG" if result == "HANG" else "FAIL")
        )
        print(f"  {name}: {status}")
