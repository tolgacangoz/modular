...
class PerChannelRMSNorm(nn.Module):
    ...
    def forward(self, x: Tensor, channel_dim: int | None = None) -> Tensor:
        """
        ...
        mean_sq = F.mean(x**2, dim=self.channel_dim, keepdim=True)
...
class LTX2VideoCausalConv3d(nn.Module):
    ...
    def forward(self, hidden_states: Tensor, causal: bool = True) -> Tensor:
        ...
        if causal:
            ...
            hidden_states = F.concat([pad_left, hidden_states], dim=2)
        else:
            pad_left = hidden_states[:, :, :1, :, :].repeat(
...
class LTXVideoDownsampler3d(nn.Module):
    ...
    def forward(self, hidden_states: Tensor, causal: bool = True) -> Tensor:
        ...
        hidden_states = hidden_states.permute(0, 1, 3, 5, 7, 2, 4, 6).flatten(
            1, 4
        )
...
class LTX2VideoDownBlock3D(nn.Module):
    ...
    def __init__(
        ...
        self.resnets = nn.sequential.ModuleList(resnets)

        self.downsamplers = None
        if spatio_temporal_scale:
            self.downsamplers = nn.sequential.ModuleList()
...
class LTX2VideoMidBlock3d(nn.Module):
    ...
    def forward(
        ...
        if self.time_embedder is not None:
            ...
            temb = temb.reshape(hidden_states.size(0), -1, 1, 1, 1)
...
class LTX2VideoEncoder3d(nn.Module):
    ...
    def __init__(
        ...
        self.down_blocks = nn.sequential.ModuleList([])
        for i in range(num_block_out_channels):
            ...
            if down_block_types[i] == "LTX2VideoDownBlock3D":
                ...
                raise ValueError(
                    f"Unknown down block type: {down_block_types[i]}"
                )
    ...
    def forward(
        self, hidden_states: Tensor, causal: bool | None = None
    ) -> Tensor:
        ...
        hidden_states = hidden_states.permute(0, 1, 3, 7, 5, 2, 4, 6).flatten(
            1, 4
        )
        ...
        last_channel = last_channel.repeat(
            1, hidden_states.size(1) - 2, 1, 1, 1
        )
        hidden_states = F.concat([hidden_states, last_channel], dim=1)
...
class LTX2VideoDecoder3d(nn.Module):
    ...
    def __init__(
        ...
        # up blocks
        num_block_out_channels = len(block_out_channels)
        self.up_blocks = nn.sequential.ModuleList([])
        for i in range(num_block_out_channels):
        ...
        if timestep_conditioning:
            self.timestep_scale_multiplier = Tensor.constant(
                torch.tensor(1000.0, dtype=DType.float32)
            )
            self.time_embedder = PixArtAlphaCombinedTimestepSizeEmbeddings(
                output_channel * 2, 0
            )
    ...
    def forward(
        ...
        if self.time_embedder is not None:
            ...
            temb = temb.reshape(hidden_states.size(0), -1, 1, 1, 1).unflatten(
                1, (2, -1)
            )
...
class AutoencoderKLLTX2Video(
    ...
    def __init__(
        ...
        latents_mean = Tensor.zeros((latent_channels,), requires_grad=False)
        latents_std = Tensor.ones((latent_channels,), requires_grad=False)
    ...
    def _encode(self, x: Tensor, causal: bool | None = None) -> Tensor:
        _batch_size, _num_channels, num_frames, height, width = x.shape

        if (
            self.use_framewise_decoding
    ...
    def _decode(
        ...
        _batch_size, _num_channels, num_frames, height, width = z.shape
    ...
    def tiled_encode(self, x: Tensor, causal: bool | None = None) -> Tensor:
        r"""Encode a batch of images using a tiled encoder.

        Args:
            x (`Tensor`): Input batch of videos.

        Returns:
            `Tensor`:
                The latent representation of the encoded videos.
        """
        _batch_size, _num_channels, _num_frames, height, width = x.shape
        ...
        for i, row in enumerate(rows):
            ...
            result_rows.append(F.concat(result_row, dim=4))

        enc = F.concat(result_rows, dim=3)[
    ...
    def tiled_decode(
        ...
        _batch_size, _num_channels, _num_frames, height, width = z.shape
        ...
        dec = F.concat(result_rows, dim=3)[
    ...
    def _temporal_tiled_encode(
        self, x: Tensor, causal: bool | None = None
    ) -> AutoencoderKLOutput:
        _batch_size, _num_channels, num_frames, height, width = x.shape
        latent_num_frames = (
            num_frames - 1
        ...
        blend_num_frames = (
            tile_latent_min_num_frames - tile_latent_stride_num_frames
        )
        ...
        for i, tile in enumerate(row):
            if i > 0:
                ...
                result_row.append(
                    tile[:, :, : tile_latent_stride_num_frames + 1, :, :]
                )

        enc = F.concat(result_row, dim=2)[:, :, :latent_num_frames]
        return enc
    ...
    def _temporal_tiled_decode(
        ...
        _batch_size, _num_channels, num_frames, _height, _width = z.shape
        num_sample_frames = (
            num_frames - 1
        ) * self.temporal_compression_ratio + 1
        ...
        dec = F.concat(result_row, dim=2)[:, :, :num_sample_frames]
