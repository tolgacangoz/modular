...
class LTX2AudioAttnBlock(nn.Module):
    ...
    def __call__(self, x: Tensor) -> Tensor:
        ...
        attn = F.bmm(q, k) * (int(channels) ** (-0.5))
        attn = F.softmax(attn, dim=2)

        v = v.reshape([batch, channels, height * width])
        attn = attn.permute(0, 2, 1)
        h_ = F.bmm(v, attn).reshape([batch, channels, height, width])
...
class LTX2AudioResnetBlock(nn.Module):
    ...
    def __call__(self, x: Tensor, temb: Tensor | None = None) -> Tensor:
        h = self.norm1(x)
...
class LTX2AudioEncoder(nn.Module):
    def __init__(
        ...
        for level in range(self.num_resolutions):
            stage = nn.Module()
            stage.block = nn.sequential.ModuleList()
            stage.attn = nn.sequential.ModuleList()
    ...
    def __call__(self, hidden_states: Tensor) -> Tensor:
        ...
        for level in range(self.num_resolutions):
            ...
            if level != self.num_resolutions - 1 and hasattr(
                stage, "downsample"
            ):
...
class LTX2AudioDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.up = nn.sequential.ModuleList()
        block_in = base_block_channels
...
class AutoencoderKLLTX2Audio(ModelMixin, AutoencoderMixin, ConfigMixin):
    ...
    def __init__(
        ...
        latents_std = Tensor.zeros((base_channels,))
        latents_mean = Tensor.ones((base_channels,))
