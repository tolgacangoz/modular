...
class PerChannelRMSNorm(Module[[Tensor], Tensor]):
    ...
    def forward(self, x: Tensor) -> Tensor:
        mean_sq = F.mean(x**2, axis=self.channel_dim, keepdims=True)
        rms = F.sqrt(mean_sq + self.eps)
        return x / rms


class LTX2VideoCausalConv3d(Module[[Tensor], Tensor]):
...
class LTX2VideoResnetBlock3d(Module[[Tensor, Tensor | None], Tensor]):
...
class LTX2VideoDecoderBlock3D(Module[[Tensor, Tensor | None], Tensor]):
...
class LTX2VideoDecoder3d(Module[[Tensor, Tensor | None], Tensor]):
...
class AutoencoderKLLTX2Video(Module[[Tensor, Tensor | None], Tensor]):
